...
public class BeerHandler {
    ...
    public Mono<ServerResponse> createBeer(ServerRequest serverRequest){
        return beerService
                .saveBeer(serverRequest.bodyToMono(BeerDTO.class).doOnNext(this::validate))
                .flatMap(beerDTO -> ServerResponse.created(UriComponentsBuilder.fromPath(BeerRouterConfig.BEER_PATH_ID)
                        .build(beerDTO.getId()))
                        .build());
